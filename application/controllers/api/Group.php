...
class Group extends REST_Controller {
    ...
    public function add_group_post($params='') {
        if($params=='add') {
            ...
            $this->form_validation->set_rules('group_leader-name', 'Leader Name', 'trim|required|xss_clean|alpha_numeric|min_length[3]');
    ...
    public function group_list_post() {
        ...
        $totalRecords =  $this->group_model->get('yes', $id, $limit, $offset, $filterData);
        $data =  $this->group_model->get('no', $id, $limit, $offset, $filterData);
    
// If $totalRecords is an array, get the first element or the specific value
if (is_array($totalRecords)) {
    $totalRecords = $totalRecords[0]; // Or use the correct index/key
}
