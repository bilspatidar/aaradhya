...
class Documentcategory extends REST_Controller {
    ...
    */
    public function __construct() {
        $this->cors_header();
        parent::__construct();
        $this->load->model('documentcategory_model');
        $this->load->helper('security');
    }

    
    public function document_category_list_post() {
        ...
        $totalRecords =  $this->documentcategory_model->get('yes', $id, $limit, $offset, $filterData);
        $data =  $this->documentcategory_model->get('no', $id, $limit, $offset, $filterData);
    ...
    public function document_category_details_get(){
        $id = $this->input->get('id') ? $this->input->get('id') : 0;
        $getTokenData = $this->is_authorized('superadmin');
    ...
    public function document_category_add_post($params='') {
        if($params=='add') {
            $getTokenData = $this->is_authorized('superadmin');
            ...
            if ($this->form_validation->run() === false) {
                ...
                if ($res = $this->documentcategory_model->create($data)) {
                    // category creation ok
                    $final = array();
                    $final['status'] = true;
                    $final['data'] = $this->documentcategory_model->get($res);
